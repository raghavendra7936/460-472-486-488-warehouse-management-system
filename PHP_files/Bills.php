...
<fieldset>
    ...
    <table>
        ...
        <?php
            $conn = mysqli_connect("localhost", "rupak", "", "WholeSale_Management");
            $sql = "SELECT transaction_information.TransactionID AS t1, customer_information.Name AS t2, product.Pname AS t3,
                    transaction_detail.Quantity AS t4, transaction_detail.Total_Amount AS t5, payment.Amount_Paid AS t6
                    FROM transaction_detail
                    JOIN transaction_information ON transaction_detail.TransactionID = transaction_information.TransactionID
                    JOIN customer_information ON customer_information.CustomerID = transaction_information.CustomerID
                    JOIN product ON product.ProductID = transaction_detail.ProductID
                    JOIN payment ON transaction_detail.TransactionID = payment.TransactionID
                    WHERE transaction_detail.TransactionID = '$TransID'";
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                <tr>
                    <td colspan="6">No results found for TransactionID: <?php echo $TransID; ?></td>
                </tr>
            <?php }
        ?>
    </table>
